import genSVG from "./tools/genSVG.js"
...
export function loadMap(containerId, { markRadius = 1, colorObj }) {
    const markers = []
    let map = null
    ...
    const colorLayer = new AMap.LabelsLayer({
        zooms: [3, 20],
        zIndex: 1000,
        collision: false,
    })
    ...
    function colorRender(data) {
        ...
        arrs.forEach(({ id, data }) => {
            const [r, g, b] = colorObj[id]
            data.forEach((item) => {
                ...
                colors.push(labelMarker)
            })
        })

        colorLayer.add(colors)
    }

    function hideColorMaskRender() {
        colorLayer.hide()
    }

    function showColorMaskRender() {
        colorLayer.show()
    }
